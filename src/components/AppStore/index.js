...
class AppStore extends Component {
  ...
  getActiveTabApp = searchedApps => {
    const {activeTabId} = this.state
    const filteredApp = searchedApps.filter(
      eachSearchdApp => eachSearchdApp.category === activeTabId,
    )
    return filteredApp
  }
  ...
  render() {
    ...
    const searchResult = this.getSerachResult()
    const filteredApp = this.getActiveTabApp(searchResult)
